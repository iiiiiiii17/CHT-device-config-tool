...
file_path = base_path / "MGS35350-16.txt"

# ------------------ GUI ------------------ #
root = tk.Tk()
root.title("設備設定工具")
root.geometry("850x500")
current_file_label = tk.Label(root, text=f"目前檔案: {file_path.name}", anchor="w")
current_file_label.pack(fill="x", padx=5, pady=2)
        ...
        with file_path.open("r", encoding="utf-8") as f:
            for line in f:
                if line.strip().lower().startswith("ip address default-management"):
                    parts = line.strip().split()
                    if len(parts) >= 5:
                        mgmt_ip_var.set(parts[3])   # 第4個欄位是 IP
                        mask_var.set(parts[4])      # 第5個欄位是遮罩
                        return
    ...
    try:
        ...
        with file_path.open("r", encoding="utf-8") as f:
            for line in f:
                ...
                # 修改 hostname
                if stripped.startswith("hostname") and new_hostname:
                    indent = line[:len(line) - len(line.lstrip())]
                    lines.append(f"{indent}hostname {new_hostname}\n")
                    hostname_found = True

                # 修改 管理IP
                elif stripped.startswith("ip address default-management") and new_ip:
                    parts = line.strip().split()
                    if len(parts) >= 5:
                        mask = parts[4] if not new_mask else new_mask  # 如果輸入遮罩就用新的
                        indent = line[:len(line) - len(line.lstrip())]
                        lines.append(f"{indent}ip address default-management {new_ip} {mask}\n")
                        ip_found = True
                    else:
...
tk.Button(button_frame, text="選擇檔案", width=12, command=choose_file).pack(side="left", padx=5)
